...
def processeur():
   ...
   # k= int(math.log(nbbit,2))
    ...
    wenable = (op_entre_deux_reg & Not(opcode99))| load | lui |op_entre_reg_et_i 
    ...
    wdata = Mux(lui,Mux(load,Mux(jump,Mux(slt|sltu,outalu,zero[:nbbit-1] + negative),Constant("0"*(nbbit-7)) + incroffset),ramdata),u)



    return offset,instr,ramWenable,wdata,waddr
